    <script>
        // === CONFIGURACIÓN ===
        const GEMINI_API_KEY = 'AIzaSyCh-Dp1tujJds1aHqFw2hxnvIzikF2InyE';
        ...
        let requisitosBasicos = ["18 a 65 años", "Peso > 110 lbs", "DUI Vigente", "Desayuno ligero"], restricciones = ["Tatuajes < 1 año", "Embarazo/Lactancia", "Alcohol 24h", "Gripe"], consejosAntes = ["Dormir 6h", "Beber agua", "Ropa cómoda"], consejosDespues = ["No fumar 2h", "No alcohol 24h", "Beber líquidos"];
        ...
        const dataHandler = { onDataChanged(data) { currentCitas = data; updateCalendar(); if(isAdminUnlocked && !document.getElementById('view-admin').classList.contains('hidden')) loadAdminData(); } };

        async function initializeApp() {
            try { await window.dataSdk.init(dataHandler); await window.elementSdk.init({}); 
            ...
            renderRequisitos(); } catch (e) { console.error(e); }
        }
        ...
        function toggleMute() { isMuted = !isMuted; document.getElementById('mute-btn').textContent = isMuted ? '🔇' : '🔊'; if(isMuted) synth.cancel(); }
        ...
        async function sendMessage(autoSpeak = false) {
            ...
            try {
                ...
                console.error(e);
                document.getElementById(loadingId).remove();
                chat.innerHTML += `<div class="chat-ai self-start bg-red-50 border border-red-100 text-red-600 p-3 rounded-xl text-xs mb-2">Error: ${e.message}</div>`;
            }
        }
